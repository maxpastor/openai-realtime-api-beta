...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  appendInputAudio(arrayBuffer) {
    if (arrayBuffer.byteLength > 0) {
      const newAudio = arrayBuffer instanceof Int16Array ? arrayBuffer : new Int16Array(arrayBuffer);
      const combinedLength = this.inputAudioBuffer.length + newAudio.length;
      const combinedBuffer = new Int16Array(combinedLength);
      combinedBuffer.set(this.inputAudioBuffer);
      combinedBuffer.set(newAudio, this.inputAudioBuffer.length);

      this.inputAudioBuffer = combinedBuffer;

      this.realtime.send('input_audio_buffer.append', {
        audio: RealtimeUtils.arrayBufferToBase64(newAudio),
      });
    }
    return true;
  }
