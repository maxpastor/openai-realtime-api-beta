...
export class RealtimeAPI extends RealtimeEventHandler {
   ...
   * @returns {RealtimeAPI}
   */
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug, event_id } = {}) {
    super();
    ...
    this.debug = !!debug;
    this.ws = null;
    this.event_id = event_id || RealtimeUtils.generateId('evt_');
  ...
  async connect({ model } = { model: 'gpt-4o-realtime-preview-2024-10-01' }) {
    ...
    if (globalThis.WebSocket) {
      ...
      ws.on('message', (data) => {
        ...
        console.log(this.event_id, JSON.stringify(message));
        this.receive(message.type, message);
      });
      return new Promise((resolve, reject) => {
        const connectionErrorHandler = () => {
